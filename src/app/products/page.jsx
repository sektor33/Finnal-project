...
import RequireAuth from "@/components/RequireAuth"; // ✅ დაამატე ეს იმპორტი
    ...
    <RequireAuth>
      <section className={styles.wrap}>
        <div className={styles.toolbar}>
          ...
          {error && (
            <span className={styles.note} style={{ color: "#dc2626" }}>
              {error}
            </span>
          )}
        </div>
          ...
          {items.map((p) => (
            <article key={p.id} className={styles.card}>
              ...
              <div className={styles.actions}>
                <Link href={`/products/details/${p.id}`} className={styles.btn}>
                  Details
                </Link>
                <button
